...
class DueReminderNotification extends Notification
{
    ...
    public function toWebPush($notifiable, $notification)
    {
        ...
        $formattedAmount = $isVoluntary
            ...
            : 'senilai Rp ' . number_format($this->due->amount, 0, ',', '.');

        return (new WebPushMessage)
            ->title('Pengingat Pembayaran Iuran')
            ->icon('/favicon.ico')
            ->body("Tagihan '{$this->due->title}' {$formattedAmount} belum lunas. Segera lakukan pembayaran.")
            ->action('Bayar Sekarang', url('/warga/dues'))
            ->options([
                'TTL' => 10000,
            ]);
    }
