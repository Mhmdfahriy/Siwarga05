...
export default function SekretarisLaporanIndex({ auth, laporans, stats, filters, recentNews }) {
    ...
    // PREFIX RUTE KHUSUS SEKRETARIS
    const routePrefix = 'sekretaris';
    ...
    const categories = ['Semua', 'Keamanan', 'Infrastruktur', 'Sosial', 'Kebersihan', 'Umum'];
    const statusOptions = [
                        ...
                        <div className="lg:col-span-3 space-y-6 order-2 lg:order-1">
                            ...
                            <div className="bg-gradient-to-br from-[#10b981] to-[#0D7A57] p-5 sm:p-6 rounded-3xl shadow-sm text-white relative overflow-hidden space-y-2">
                                <div className="flex items-center gap-2">
                                    <svg className="w-4 h-4 text-white shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    <h4 className="font-bold text-xs uppercase tracking-wider">Administrasi Kesekretariatan</h4>
                                </div>
                                <p className="text-xs text-emerald-50 leading-relaxed">
                                    Pantau dan kelola aduan warga terkait infrastruktur, keamanan, sosial, dan kebersihan lingkungan.
                                </p>
                            </div>
                        </div>
                                                    ...
                                                    <div className="flex gap-3">
                                                        ...
                                                        <div className="bg-white p-3.5 rounded-2xl rounded-tl-sm shadow-xs border border-gray-100 text-xs text-gray-800 space-y-2 w-full">
                                                            <div className="font-bold text-[10px] text-blue-600 mb-0.5">
                                                                {activeLaporan.user?.name || 'Warga'} (WARGA)
                                                            </div>
                                                        ...
                                                        return (
                                                            <div key={komentar.id} className={`flex gap-3 ${isMine ? 'flex-row-reverse' : ''}`}>
                                                                <div className={`p-3.5 rounded-2xl shadow-xs border text-xs max-w-[85%] space-y-1 ${
                                                                    ...
                                                                    {!isMine && komentar.user && (
                                                                        <div className="font-bold text-[10px] text-emerald-700 mb-0.5">
                                                                            {komentar.user.name} ({komentar.user.role?.replace('_', ' ').toUpperCase()})
                                                                        </div>
                                                                    )}
