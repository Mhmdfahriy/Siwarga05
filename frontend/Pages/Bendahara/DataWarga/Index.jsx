...
import { Search, RotateCcw, User, ShieldCheck, ShieldAlert, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, Eye, Home, Users } from 'lucide-react';
...
export default function DataWargaIndex({ user, warga, filters, canResetNik, totalWarga }) {
  ...
  const handleResetNik = (userId, name) => {
    setModalConfig({
      isOpen: true,
      title: 'Reset NIK Warga',
      message: `Reset NIK milik ${name}? Warga tersebut perlu mengisi ulang NIK dari halaman profilnya.`,
      type: 'warning',
      onConfirm: () => {
        router.put(route(`${prefix}.data-warga.reset-nik`, userId), {}, { 
          preserveScroll: true,
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-gray-100 pb-5">
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-500 mt-1">Kelola data kependudukan dan informasi rumah warga lingkungan RT 05.</p>
            </div>
                    ...
                    return (
                      <React.Fragment key={house.house_id}>
                        ...
                        <tr className="text-xs sm:text-sm hover:bg-gray-50/50 transition">
                          ...
                          <td className="px-4 py-3.5 whitespace-nowrap">
                            {(house.user_id || house.resident_id) ? (
                              <div className="flex items-center gap-3">
                                <Link
                                  href={route(`${prefix}.data-warga.show`, house.user_id ?? house.resident_id)}
                                  className="text-gray-400 hover:text-[#0D7A57] transition"
                                  title="Lihat Akun"
                                >
                                  <Eye className="w-4 h-4" />
                                </Link>
                                {canResetNik && house.has_nik_user && house.user_id && (
                                  <button
                                    onClick={() => handleResetNik(house.user_id, house.kepala_keluarga)}
